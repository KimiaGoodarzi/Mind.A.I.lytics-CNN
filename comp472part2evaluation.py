...
"""Part 3 Modified Evaluation"""

!gdown 1NV66-sGd1ezZuOrLntsyog5lm6IUoFhR

!gdown 1aWSSMoNBtDrszbEhn6oLFA2eqhkHRryp
!unzip 'ModifiedCOMP472Data.zip'

modelp3 = nn.Sequential(
    nn.Conv2d(1, 32, kernel_size=3, padding=1),
    nn.ReLU(),
    nn.BatchNorm2d(32),
...
modelp3.load_state_dict(torch.load('/content/modifiedDatasetbestModel.pth', map_location=torch.device('cpu')))

path = '/content/ModifiedCOMP472Data'
transform = v2.Compose([v2.ToImage(),v2.Grayscale(),v2.ToDtype(torch.float32, scale=True)])
full_modified_dataset = ImageFolder(path,transform)
class_names = full_modified_dataset.classes
...
batch_size = 32 #Batch size for model
# Split dataset indices
train_indices, test_indices = train_test_split(list(range(len(full_modified_dataset))), test_size=ttsplit, random_state=42)
train_indices, val_indices = train_test_split(train_indices, test_size=vtsplit, random_state=42)
# Create subsets
train_set = Subset(full_modified_dataset, train_indices)
val_set = Subset(full_modified_dataset, val_indices)
test_set = Subset(full_modified_dataset, test_indices)
# Create data loaders
train_loader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, shuffle=True)
test_loader = torch.utils.data.DataLoader(test_set, batch_size=batch_size, shuffle=False)
val_loader = torch.utils.data.DataLoader(val_set, batch_size=batch_size, shuffle=False)
...
#Compute the confusion matrix
pred, true = evaluate(modelp3,test_loader)
